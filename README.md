...
- **Extraction**: Using the DataExtractor class to retrieve data from various sources.
- **Transformation**: Using the DataCleaning class to clean and preprocess the extracted data.
- **Loading**: Using the DataLoader class to load the cleaned data into the PostgreSQL database.
...
- Create the Python script **database_utils.py** containing a **DatabaseConnector** class to manage database connections and uploads.
- Develop the **data_extraction.py** script with a **DataExtractor** class to handle data extraction from CSV, JSON, PDF, and API sources.
- Create the **data_cleaning.py** script with a **DataCleaning** class to clean and preprocess data before it is stored in the database.
- Add the **data_loading.py** script with a **DataLoader** class to handle loading the cleaned data into the PostgreSQL database.

## Milestone 2: Data Extraction and Cleaning

### Task 1: Extract and Clean User Data

**Goal**: Extract historical user data from an AWS RDS database, clean it, and store it in the **dim_users** table.

**Steps:**

- **Extract**: Use the extract_rds_table method in DataExtractor to connect to the RDS instance and extract the user data.
- **Clean**: Implement the clean_user_data method in DataCleaning to address null values, date errors, and data type inconsistencies.
- **Load**: Upload the cleaned data to the PostgreSQL database using the load_to_db method in DataLoader.

### Task 2: Extract and Clean Card Details
...
- **Load**: Upload the cleaned data to the **dim_card_details** table in the PostgreSQL database using the **load_to_db** method.

### Task 3: Extract and Clean Store Data
...
- **Load**: Upload cleaned data to the PostgreSQL database using the **load_to_db** method.

### Task 4: Extract and Clean Product Data
...
- **Load**: Upload the cleaned data to the PostgreSQL database using the **load_to_db** method.

### Task 5: Extract and Clean Orders Data

**Goal**: Extract orders data from AWS RDS, clean it, and store it in the orders_table.

**Steps:**

- **Extract**: Use the **extract_rds_table** method in **DataExtractor** to fetch orders data.
...
### Task 6: Extract and Clean Date Events Data

**Goal**: Extract date events data from a JSON file on S3, clean it, and store it in dim_date_times.

**Steps:**

- **Extract**: Use **extract_json_from_url** in DataExtractor to read the JSON file.
- **Clean**: Implement **clean_date_events_data** in DataCleaning to ensure the timestamp and time period data is correctly formatted.
- **Load**: Upload the cleaned data to the PostgreSQL database using the **load_to_db** method.
